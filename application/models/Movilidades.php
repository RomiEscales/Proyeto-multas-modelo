...
class Movilidades extends CI_Model
{
	...
	function buscarPersonas($nrDoc){
    	$query= $this->db->get_where('personaunsj',array('dni'=>$nrDoc));
		if ($query->num_rows() != 0){
			return 1;
		} else {
			return 0;
    ...
    function buscarPersonaUnsj($nrDoc){

    	$query= $this->db->get_where('personaextranjero',array('dni'=>$nrDoc));
		if ($query->num_rows() != 0){
			return 1;
		} else {
			return 0;
