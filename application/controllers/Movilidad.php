...
class Movilidad extends CI_Controller {
	...
	public function index($permission)
	{
		...
		$this->load->view('movilidad/list', $data);
	}
	...
	public function cargarexp($permission)
	{
		...
		$data['permission'] = $permission;
		//$this->load->view('movilidad/view_', $data);
		$this->load->view('movilidad/registrar', $data);
	}

	public function registrarMovilidad($permission){
	...
	{
	    $nrDoc=$_POST['nroDocumento'];

    	$result = $this->Movilidades->buscarPersonas($nrDoc);
    	if ($result == 1){
    		//$arre['ext']=$result;
    		$arre=$result;
    	}
		...
		{
			//No esta en la table persona unsj 
			//Ahora voy a buscar en la tabla personaextranjer 
			//$arre['ext']=0;
			
			$unsj = $this->Movilidades->buscarPersonaUnsj($nrDoc);
			if ($unsj == 1){
